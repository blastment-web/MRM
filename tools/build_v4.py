...
HERO_MEDIA = f'''    <!-- V4-2) 히어로 배경 영상. loop 속성 대신 JS 로 3~8초 구간만 반복한다.
    ...
    <video id="heroVideo" aria-hidden="true" autoplay muted playsinline preload="auto" hidden>
      ...
      <source src="hero.webm#t=3,8" type="video/webm">
...
HERO_JS = '''  /* ---------------------------------------------------------------
  ...
  (function heroVideo(){
    ...
    /* 별자리를 접는 조건은 하나다 — 영상이 "실제로 그림을 내고 있는가".
       canplay 만 믿으면 안 된다. 코덱·Range 미지원·디코더 실패에서도 canplay 는 뜨는데
       화면에는 검은 판만 남는다. 그 상태로 별자리를 접으면 첫 화면이 통째로 검어진다.
       그래서 timeupdate 가 실제로 진행되고 videoWidth 가 잡힐 때만 드러낸다. */
    var lastAdvance = 0;
    /* 조건은 "그림이 실제로 있는가" 다.
       readyState >= 2(HAVE_CURRENT_DATA) 면 디코딩된 프레임이 있어 화면에 그려진다.
       videoWidth > 0 은 그 프레임이 진짜 영상이라는 뜻이다(코덱 실패면 0 이다).
       재생 여부는 넣지 않는다 - 회사 정책으로 자동재생이 막혀도 첫 프레임은 나오므로
       검은 판이 되지 않는다. 자동재생이 막힌 경우는 아래에서 첫 조작에 다시 시도한다. */
    function canShow(){ return v.readyState >= 2 && v.videoWidth > 0; }
    function reveal(){
      if(shown || dead || !canShow()) return;
      shown = true;
      v.hidden = false;
      if(veil) veil.hidden = false;
      document.body.classList.add("herovid");   /* 이때만 별자리를 접는다 */
    }
    /* 드러낸 뒤에 멈춰 버리면 되돌린다. 되돌리면 별자리가 다시 히어로가 된다.
       dead 로 만들지 않는 이유 — 잠깐 끊긴 것이면 다음 timeupdate 에 다시 드러난다. */
    function unreveal(){
      if(!shown) return;
      shown = false;
      v.hidden = true;
    ...
    v.addEventListener("loadeddata", function(){ lastAdvance = Date.now(); reveal(); });
    /* 자동재생이 막혔다가 나중에 시작하면 0초부터 흐른다. 그때 한 번만 구간으로 옮긴다.
       매 프레임 확인하지 않는 이유 - Range 를 못 쓰는 서버에서는 seek 마다 다시 받느라
       버퍼링이 끝나지 않는다. 한 번이면 충분하다. */
    var fixedStart = false;
    v.addEventListener("playing", function(){
      if(fixedStart) return;
      fixedStart = true;
      if(v.currentTime < START - 0.5) seekStart();
    });
    ...
    v.addEventListener("timeupdate", function(){
      ...
      if(canShow()){ lastAdvance = Date.now(); reveal(); }
    });
    /* 드러낸 뒤 6초 넘게 진행이 없으면(멈춤·버퍼링) 별자리로 되돌린다.
       일부러 세운 경우(스크롤 중·화면 밖·탭 비활성)는 제외한다. */
    /* 이벤트에만 기대지 않는다. preload="auto" 라 스크립트가 붙기 전에 loadeddata 가
       이미 지나갔을 수 있고, 자동재생이 막히면 timeupdate 도 오지 않는다.
       그러면 조건이 충족됐는데도 영영 드러나지 않는다. 짧은 주기로 직접 확인한다. */
    setInterval(function(){
      if(dead) return;
      if(!shown){ if(canShow()){ lastAdvance = Date.now(); reveal(); } return; }
      if(v.paused || document.hidden || !visible) return;
      /* 재생 중인데 6초 넘게 진행이 없다 = 버퍼링으로 멈춘 것. 별자리로 되돌린다. */
      if(Date.now() - lastAdvance > 6000) unreveal();
    }, 500);
    reveal();   /* 이미 준비돼 있었다면 기다릴 이유가 없다 */
    /* 자동재생이 막힌 경우 첫 조작에 한 번 더 시도한다. 첫 프레임은 이미 떠 있다. */
    ...
    /* <source> 하나가 실패하는 것은 정상이다 - "다음 것을 써라" 라는 뜻이다.
       그것을 치명적 실패로 처리하면, 뒤에 멀쩡한 소스가 있어도 영상이 영영 안 뜬다.
       (hero.mp4 가 없는 상태에서 첫 소스가 404 나면 바로 이 일이 벌어졌다.)
       진짜 실패는 <video> 자신의 error 다 - 모든 소스를 다 써 보고 난 뒤에만 뜬다.
       capture 로 듣지 않는 것도 같은 이유다. capture 면 자식 <source> 의 error 까지 잡힌다. */
    v.addEventListener("error", function(e){ if(e.target === v) fail(); });
