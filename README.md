...
| **MAPS V4-2** | V4-1 + 첫 화면(히어로만) / 대시보드(3패널) 두 화면으로 분리. 히어로는 **배경 영상**이되, 영상을 못 받아 오면 **별자리가 그대로 히어로로 남는다**(빈 첫 화면이 생기지 않는다). `releases/MAPS-V4-2/`. |
...
| 섹션 | ID | 내용 |
|------|-----|------|
| 대시보드(Hero) | `#secHome` | 메인 카피 + 현황·차트·공지 3패널. 배경은 V4-1 파티클 / V4-2 는 파티클 위에 영상(영상이 받아질 때만) |
| 대시보드 (V4-2 전용) | `#secDash` | V4-2 에서 3패널이 이 섹션으로 분리됨 |

### V4-2 히어로가 안 뜨던 이유 (실측으로 잡은 것)

`<source>` 하나가 실패하는 것은 "다음 것을 써라" 라는 뜻인데, 그 오류를 치명적 실패로
처리하고 있었다. `hero.mp4` 가 없는 상태에서 첫 `<source>` 가 404 나면 그 순간
**뒤에 멀쩡한 소스가 있어도 영상이 영영 뜨지 않았다.** 이제 `<video>` 자신의 `error`
(모든 소스를 다 써 본 뒤에만 뜬다) 만 치명적으로 본다.

영상은 `1_서버켜기.bat` 이 처음 한 번 받아 이 PC 에 두고(`hero.mp4`) 그 뒤로는 자기
서버가 내보낸다 — 매번 외부로 나가면 회사 프록시에 막히거나 느려서 안 뜬다.
`hero.webm` 도 받는다.

V4-2 의 히어로는 **두 겹**이다. 아래에 V4-1 과 같은 별자리 캔버스(`#pxNet`)가 늘 깔리고,
영상(`#heroVideo`)은 `canplay` 가 뜬 뒤에야 그 위로 드러나면서 별자리를 접는다
(`body.herovid`). 영상이 막힌 망·느린 원격 화면에서는 별자리가 그대로 히어로가 되므로
**첫 화면이 비는 일이 없다.** 같은 폴더에 `hero.mp4` 를 두면 외부 왕복 없이 그것을 쓴다.
